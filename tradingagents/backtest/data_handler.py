...
class HistoricalDataHandler:
    ...
    def load_data(
        ...
        for ticker in tqdm(
            tickers, desc="Loading data", disable=not self.config.progress_bar
        ):
            # Validate ticker
    ...
    def _load_from_csv(
        ...
        csv_path = (
            Path(self.config.custom_params.get("csv_dir", "data")) / f"{ticker}.csv"
        )
    ...
    def get_data_at(
        self, ticker: str, timestamp: datetime, lookback: Optional[int] = None
    ) -> pd.DataFrame:
        """
    ...
    def get_price_at(
        self, ticker: str, timestamp: datetime, price_type: str = "close"
    ) -> Decimal:
        """
    ...
    def align_data(
        self, tickers: Optional[List[str]] = None, method: str = "inner"
    ) -> pd.DataFrame:
        """
    ...
    def get_trading_days(
        ...
        trading_days = all_dates[(all_dates >= start_date) & (all_dates <= end_date)]
