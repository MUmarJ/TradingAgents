...
class ExecutionSimulator:
    ...
    def execute_order(
        ...
        if not self._can_fill_order(order, current_price):
            order.status = OrderStatus.REJECTED
            logger.debug("Order rejected - price conditions not met")
            return order

        # Calculate fill price with slippage
        fill_price = self._calculate_fill_price(order, current_price, current_volume)
        ...
        if self.config.partial_fills:
            fill_quantity = self._calculate_partial_fill(order.quantity, current_volume)
        ...
        if order.side == OrderSide.BUY:
            ...
            if total_required > available_capital:
                if self.config.partial_fills:
                    ...
                    fill_quantity = min(
                        fill_quantity, affordable_quantity.quantize(Decimal("1"))
                    )
    ...
    def _calculate_fill_price(
        self, order: Order, current_price: Decimal, current_volume: Decimal
    ) -> Decimal:
        """
        ...
        if self.config.slippage_model == SlippageModel.FIXED:
            ...
            slippage = self._calculate_volume_slippage(order, base_price, current_volume)
    ...
    def _calculate_fixed_slippage(self, order: Order, base_price: Decimal) -> Decimal:
        """Calculate fixed percentage slippage."""
        return self.config.slippage

    def _calculate_volume_slippage(
        self, order: Order, base_price: Decimal, current_volume: Decimal
    ) -> Decimal:
    ...
    def _calculate_spread_slippage(self, order: Order, base_price: Decimal) -> Decimal:
    ...
    def _calculate_partial_fill(
        self, order_quantity: Decimal, current_volume: Decimal
    ) -> Decimal:
        """
...
def create_market_order(
    ticker: str, side: OrderSide, quantity: Decimal, timestamp: datetime
) -> Order:
    """
