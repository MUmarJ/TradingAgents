...
class AlpacaBroker(BaseBroker):
        ...
        with self._lock:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        "Successfully connected to Alpaca (Account: %s)", account_number
                    )
                    return True
    ...
    def get_account(self) -> BrokerAccount:
        ...
        try:
            ...
            logger.debug(
                "Account retrieved: cash=%.2f, buying_power=%.2f",
                account.cash,
    ...
    def get_position(self, symbol: str) -> Optional[BrokerPosition]:
        ...
        try:
            ...
            logger.debug(
                "Position retrieved: %s qty=%s, pnl=%.2f",
                symbol,
                position.quantity,
    ...
    def submit_order(self, order: BrokerOrder) -> BrokerOrder:
        ...
        if order.order_type in [OrderType.LIMIT, OrderType.STOP_LIMIT]:
            if order.limit_price is None:
                logger.error(
                    "Limit price required for %s order on %s",
        ...
        if order.order_type in [OrderType.STOP, OrderType.STOP_LIMIT]:
            if order.stop_price is None:
                logger.error(
                    "Stop price required for %s order on %s",
                    order.order_type.value,
                    order.symbol,
                )
        ...
        try:
            ...
            logger.info(
                "Order submitted successfully: %s (ID: %s, Status: %s)",
                order.symbol,
                order.order_id,
                order.status.value,
            )
    ...
    def get_order(self, order_id: str) -> Optional[BrokerOrder]:
        ...
        try:
            ...
            logger.debug(
                "Order retrieved: %s status=%s, filled=%s",
                order_id,
                order.status.value,
                order.filled_qty,
            )
